    ...
    public partial class tb_TemperatureInfoBYDDAL
    {
        ...
        public bool InsertList(List<tb_TemperatureInfoBYD> lstTemperature)
        {
            ...
            for (int i = 0; i < lstTemperature.Count; i++)
            {
                ...
                sb.Append(string.Format(@" insert into tb_TemperatureInfo (RecordTime, Temperature1,Temperature2,Temperature3,Temperature4, LayerNum, CarrierId,StationNum) 
                                            values ('{0}', {1}, {2}, {3}, {4},{5},{6},{7}) \r\n ", temp.RecordTime, temp.Temperature1, temp.Temperature2,
        ...
        public DataTable GetTemperature(string carrierId, string stationNum, DateTime? dtBegin, DateTime? dtEnd, int PageIndex, int PageCount, int layCount, ref int totalRecord)
        {
            ...
            sqlstr = string.Format(@"select ttt.Eno '生产编号', --CarrierId
                                                    ttt.WorkStationNo-1001 '腔体编号',--StationNum-1001
                                                    ttt.CarNo '载体编号',--CarrierNum
                                                    ...
                                                    {3}
                                                     from ( 
                                                           ...
                                                           from( 
                                                           select t2.Eno, t1.SamplingTime,t1.TemperatureControl,t1.WorkStationNo,t1.CarSystemID,t1.VacPV,LayerNum 
                                                            ,t2.CarNo,ROW_NUMBER() over (order by t1.SamplingTime desc) rowNooo
